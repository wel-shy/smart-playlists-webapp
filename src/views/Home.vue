        ...
        div.columns.is-mobile.subscription(
          ...
          div.column.is-4.has-text-right
            button(
              ...
              :class="[{'is-loading': playlist.isLoading},buttonClass]"
            ) Active
...
const apiURL = 'https://api.smart-lists.dwelsh.uk/api';
  ...
  methods: {
    async toggleActive(index) {
      ...
      if (!this.playlists[index].active) {
        ...
        const url = `${apiURL}/subs/${this.playlists[index].id}`;
